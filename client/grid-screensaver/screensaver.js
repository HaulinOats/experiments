 ...
 * Codepen to help with defining grids: https://codepen.io/midgitsuu/full/mQMVKJ/
...
function gridScreensaver(attachElement, imageArr, options){
  let opacityDuration = 1;
  let pageChangeTimeout = 40;
  let allowKenBurnsScaling = true;
  let closeOnClick = false;
  let bindStartQuerySelector = false;
  let startCallback = false;
  let loadCallback = false;
  let stopCallback = false;
  let gridLayoutsId = 0;
  if(options){
    opacityDuration = 'opacityDuration' in options ? Number(options.opacityDuration) : opacityDuration;
    pageChangeTimeout = 'pageChangeTimeout' in options ? Number(options.pageChangeTimeout) : pageChangeTimeout;
    allowKenBurnsScaling = 'allowKenBurnsScaling' in options ? Boolean(options.allowKenBurnsScaling) : allowKenBurnsScaling;
    closeOnClick = 'closeOnClick' in options ? Boolean(options.closeOnClick) : closeOnClick;
    bindStartQuerySelector = 'bindStartQuerySelector' in options && options.bindStartQuerySelector ? options.bindStartQuerySelector : bindStartQuerySelector;
    startCallback = 'startCallback' in options && typeof options.startCallback === "function" ? options.startCallback : startCallback;
    loadCallback = 'loadCallback' in options && typeof options.loadCallback === "function" ? options.loadCallback : loadCallback;
    stopCallback = 'stopCallback' in options && typeof options.stopCallback === "function" ? options.stopCallback : stopCallback;
    gridLayoutsId = 'gridLayoutsId' in options ? Number(options.gridLayoutsId) : gridLayoutsId;
  }
  ...
  function stop(){
    ...
    let allGridItems = document.querySelectorAll('.mvwsct_ui_gridScreensaverInner_gridItem_inner');
